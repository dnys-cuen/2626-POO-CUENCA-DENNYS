...
def mostrar_menu() -> None:
    ...
    print("7. Cargar datos desde JSON")
    print("-" * 50)
    print("8. Salir")
    print("=" * 50)
...
def registrar_bebida(restaurante: Restaurante) -> None:
    ...
    try:
        ...
        if restaurante.registrar_producto(bebida):
            ...
            print("  (Guardada automáticamente en JSON)")
        else:
            print(f"✗ Error: Ya existe un producto con el código '{codigo}'.")
...
def guardar_datos_json(restaurante: Restaurante) -> None:
    """
    Guarda todos los productos y clientes en archivos JSON con estructura diccionario.

    Args:
        restaurante (Restaurante): Instancia del servicio restaurante.
    """
    try:
        if restaurante.guardar_datos():
            print("\n✓ Datos guardados exitosamente en archivos JSON.")
            print("  Productos guardados en: datos/productos.json")
            print("  Clientes guardados en: datos/clientes.json")
        else:
...
def cargar_datos_json(restaurante: Restaurante) -> None:
    """
    Carga todos los productos y clientes desde archivos JSON.

    Args:
        restaurante (Restaurante): Instancia del servicio restaurante.
    """
    try:
        if restaurante.cargar_datos():
            cantidad_productos = restaurante.obtener_cantidad_productos()
            cantidad_clientes = restaurante.obtener_cantidad_clientes()
            print("\n✓ Datos cargados exitosamente desde archivos JSON.")
            print(f"  Productos cargados: {cantidad_productos}")
            print(f"  Clientes cargados: {cantidad_clientes}")
        else:
...
def main() -> None:
    ...
    Carga datos al iniciar si existen archivos previos.
    """
    restaurante = Restaurante()
    
    print("\n¡Bienvenido al Sistema de Restaurante!")
    print("Buscando archivos de datos previos...")
    
    if restaurante.cargar_datos():
        cantidad_prod = restaurante.obtener_cantidad_productos()
        cantidad_cli = restaurante.obtener_cantidad_clientes()
        if cantidad_prod > 0 or cantidad_cli > 0:
            print(f"✓ Datos cargados: {cantidad_prod} productos, {cantidad_cli} clientes")
    else:
        print("℘ No se encontraron datos previos. Comenzando con datos vacíos.")
    ...
    while True:
        ...
        if opcion == "1":
            ...
            guardar_datos_json(restaurante)

        elif opcion == "7":
            cargar_datos_json(restaurante)
            ...
            print("✗ Error: Opción no válida. Por favor, seleccione una opción del 1 al 8.")
